...
class EmailTemplatesController extends Controller
{
    ...
    {
        $all = DB::select("select id, content from email_templates");
    ...
    {
        $data = $request->block_id;

        if($data == 0)
        {
            $all = DB::select("select * from email_templates");

            for($i =0; $i <count($all); $i ++ )
            {
                $query = DB::update("update email_templates set status = ?",[0]);
            }
        }
        else{
            $query = DB::update("update email_templates set status = ? where id=?",[0,$data]);
        }   
        return response()->json([
            'success' => 'true',
            'proxy' => 'false',
            'reason' => 'Proxy detected [1]',
        ]);
    }
